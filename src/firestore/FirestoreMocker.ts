...
/**
 * Callback function to get a wanted id for autogenerated document during a test. This
 * makes it possible to make idempotent test with values that are always a same on each
 * call.
 *
 * @param collectionPath A path to a collection where the new document will be generated
 * @returns a document id for a new document
 */
export type GetDocumentIdCallback = (collectionPath: string) => string;
/**
 ...
 *
 * With mocker, you can alter the database state without causing any state change triggering. You should use its commands
...
export interface FirestoreMocker extends Mocker {
   ...
   *
   * @param collectionPath collection path, all sub collections and documents will be autogenerated if they do not exist
   ...
   * @param documentPath Path into the document. All subcollections and documents will be autogenerated, if they do not exist
  ...
  /**
   * Load project's security rules. After this, all operations are checked against actual firestore security rules
   * @param filePath File path to project's security rules. If path is not given, rules are tried to be read from <project-root-dir>/firestore.rules -file.
   * @throws FileNotFoundError if rules are not found from given path
   */
   ...
   * @memberof FirestoreMocker
   */
  setNextDocumentIds(docIds: string[] | GetDocumentIdCallback): void;
   ...
   * If there are no set ids available, a new random id will be generated.
   *
   * @param collectionPath Path to the collection where the document with a new id will be created
   * @returns {string}
   * @memberof FirestoreMocker
   */
